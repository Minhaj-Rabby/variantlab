 ...
 *
 * These are exercised in isolation — the UI component test lives in
 * `overlay.test.tsx`.
 */
...
describe("summarize", () => {
  it("handles every EngineEvent type", () => {
    ...
    const ready = events[0] as EngineEvent;
    const changed = events[3] as EngineEvent;
    const error = events[7] as EngineEvent;
    expect(summarize(ready)).toContain("1 experiments");
    expect(summarize(changed)).toContain("\u2192");
    expect(summarize(error)).toContain("boom");
  });
});
